...
local function getRootPart(character)
	...
	if not root then
		pcall(function() root = character:WaitForChild("HumanoidRootPart", 5) end)
	end
	return root
end
...
-------------------------------
-- TELEPORT ITEMS TO QUOTA CHECKER (Spawned Loot Items)
-------------------------------
local TeleportSection = Tabs.Functions:AddSection("Teleport Items")
TeleportSection:AddButton({
	Title = "Teleport Items to Quota Checker",
	Callback = function()
		-- Ziel: Flooring, Child von QuotaChecker (unter Spawn Area.Important.QuotaChecker)
		local spawnArea = Workspace:FindFirstChild("Spawn Area")
		if not spawnArea then return end
		local important = spawnArea:FindFirstChild("Important")
		if not important then return end
		...
		-- Suche in QuotaChecker nach Flooring
		local flooring = quotaChecker:FindFirstChild("Flooring")
		if not flooring then
			warn("Flooring in QuotaChecker nicht gefunden!")
			return
		end
		local targetPart
		if flooring:IsA("BasePart") then
			targetPart = flooring
		elseif flooring:IsA("Model") then
			targetPart = flooring.PrimaryPart or flooring:FindFirstChildWhichIsA("BasePart")
		end
		...
		if spawnedLootFolder then
			for _, item in ipairs(spawnedLootFolder:GetChildren()) do
				...
				if basePart then
					-- Teleportiere exakt auf die Flooring-Position
...
-------------------------------
-- TELEPORT SHOP ITEMS TO ITEM CHECKER (Shop Items)
-------------------------------
local function TeleportShopItemsToItemChecker()
	...
	if shopItems then
		for _, obj in ipairs(shopItems:GetChildren()) do
			local base = nil
			if obj:IsA("BasePart") then
				base = obj
			elseif obj:IsA("Model") then
			...
			if base then
				-- Teleportiere exakt auf die Hitbox des ItemCheckers
				base.CFrame = hitbox.CFrame
...
FunctionsSection:AddButton({
	Title = "Auto Shopping",
	Callback = function()
		TeleportShopItemsToItemChecker()
	end
})
